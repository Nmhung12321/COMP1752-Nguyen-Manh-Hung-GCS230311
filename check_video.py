...
class CheckVideos:
    ...
    def create_widgets(self, window):
        ...
        self.video_txt = tk.Text(window, width=24, height=10, wrap="none")
    ...
    def check_video_clicked(self):
        ...
        if name:
            ...
            reactions = lib.get_reactions(key)
            if reactions:
                reactions_str = ", ".join(reactions)  # Convert reactions to a string
            else:
                reactions_str = "No reactions"
            video_details = f"{name}\n{director}\nrating: {rating}\nplays: {play_count}\nReactions: {reactions_str}"
            set_text(self.video_txt, video_details)  # Display the details in the text areaails)  # Display the details in the text area
